...
namespace WpfMailSender.lib
{
    public class MailSchedulerService : IMailScheduler
    {
        ...
        private async Task ProcessTaskAsync(SchedulerTask task, CancellationToken Cancel)
        {
            ...
            var sender = _MailService.GetSender(server.Addres, server.Port, server.UseSSL, server.Login, server.Password);

            await sender.SendParallelAsync(
                task.Sender.SenderAdress,
                task.Recipient.Select(r => r.RecipientAdress),
                task.Message.Title,
                task.Message.Text,
                Cancel)
